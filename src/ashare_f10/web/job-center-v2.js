(() => {
  ...
  function renderJobCard(job) {
    ...
    const expanded = center.expanded.has(job.job_id);
    const errorSummary = job.errors?.length
      ? `<details class="job-error"><summary>任务错误（${job.errors.length}）</summary><pre>${escape(job.errors.join("\n"))}</pre></details>`
      : "";
    return `
  ...
  function renderJobs() {
    ...
    const existingCards = new Map(
      [...container.querySelectorAll("[data-job-card]")].map((card) => [card.dataset.jobCard, card]),
    );
    const fragment = document.createDocumentFragment();
    for (const job of center.jobs) {
      const card = cardFromHtml(renderJobCard(job));
      const existing = existingCards.get(job.job_id);
      if (existing && center.expanded.has(job.job_id)) {
        const oldGroups = existing.querySelector(".job-groups");
        const newGroups = card.querySelector(".job-groups");
        if (oldGroups && newGroups) {
          newGroups.replaceWith(oldGroups);
          oldGroups.classList.add("active");
        }
      }
    ...
    for (const job of center.jobs) {
      if (!center.expanded.has(job.job_id)) continue;
      const version = `${job.updated_at_utc || ""}:${job.status}:${job.retry_count || 0}`;
      const force = activeStatuses.has(job.status) && center.groupVersions.get(job.job_id) !== version;
      center.groupVersions.set(job.job_id, version);
      loadGroups(job.job_id, force);
    }
  }
  ...
  function readGroupUi(jobId) {
    const container = node(`job-groups-${jobId}`);
    const saved = center.groupUi.get(jobId) || { q: "", status: "", scrollTop: 0 };
    if (!container) return saved;
    const wrapper = container.querySelector(".subtask-table-wrap");
    return {
      q: container.querySelector("[data-group-q]")?.value ?? saved.q,
      status: container.querySelector("[data-group-status]")?.value ?? saved.status,
      scrollTop: wrapper?.scrollTop ?? saved.scrollTop,
    };
  }

  function filteredGroups(jobId, ui) {
    const data = center.groups.get(jobId) || [];
    const q = (ui.q || "").trim().toLowerCase();
    return data.filter((group) => {
      if (ui.status && group.status !== ui.status) return false;
      if (!q) return true;
  ...
  function renderGroups(jobId) {
    const container = node(`job-groups-${jobId}`);
    if (!container || !center.expanded.has(jobId)) return;
    const ui = readGroupUi(jobId);
    center.groupUi.set(jobId, ui);
    const job = center.jobs.find((item) => item.job_id === jobId);
    const active = job ? activeStatuses.has(job.status) : false;
    const rows = filteredGroups(jobId, ui);
    const allCount = (center.groups.get(jobId) || []).length;
    container.innerHTML = `
      <div class="group-toolbar">
        <label>过滤子任务 <input data-group-q placeholder="接口、区块或错误" value="${escape(ui.q)}" /></label>
        <label>状态
          <select data-group-status>
            <option value="">全部</option>
            <option value="FAILED" ${ui.status === "FAILED" ? "selected" : ""}>失败</option>
            <option value="SUCCESS" ${ui.status === "SUCCESS" ? "selected" : ""}>成功</option>
            <option value="RUNNING" ${ui.status === "RUNNING" ? "selected" : ""}>运行中</option>
            <option value="RETRYING" ${ui.status === "RETRYING" ? "selected" : ""}>重试中</option>
            <option value="PENDING" ${ui.status === "PENDING" ? "selected" : ""}>等待</option>
          </select>
        </label>
      ...
      <div class="subtask-table-wrap">
        <table class="subtask-table">
          ...
          <tbody>${rows.map((group) => {
            ...
            const retrying = group.status === "RETRYING" || group.status === "RUNNING";
            return `<tr class="subtask-${escape(group.status.toLowerCase())}">
              ...
              <td><button type="button" data-group-retry data-job-id="${escape(jobId)}" data-group-id="${escape(group.group_id)}" ${active || retrying ? "disabled" : ""}>${retrying ? "执行中" : "重新执行"}</button></td>
            </tr>`;
          }).join("")}</tbody>
        </table>
      </div>`;
    const wrapper = container.querySelector(".subtask-table-wrap");
    if (wrapper) wrapper.scrollTop = ui.scrollTop || 0;
  }

  async function loadGroups(jobId, force = false) {
    if (!center.expanded.has(jobId)) return;
    if (!force && center.groups.has(jobId)) {
      renderGroups(jobId);
      return;
    }
    if (center.groupRequests.has(jobId)) return center.groupRequests.get(jobId);
    const request = (async () => {
      try {
        ...
        center.groupRequests.delete(jobId);
      }
    })();
    center.groupRequests.set(jobId, request);
    return request;
  }

  function markJobRetrying(jobId) {
    const job = center.jobs.find((item) => item.job_id === jobId);
    if (!job) return;
    job.status = "RETRYING";
    job.message = "正在重新执行所选子任务";
    renderJobs();
  }
  ...
  async function retryGroup(jobId, groupId) {
    const groups = center.groups.get(jobId) || [];
    const group = groups.find((item) => item.group_id === groupId);
    if (group) {
      group.status = "RETRYING";
      group.attempt_count = Number(group.attempt_count || 0) + 1;
      group.errors = [];
      renderGroups(jobId);
    }
    markJobRetrying(jobId);
    try {
      ...
      notify("已开始重新执行该子任务");
      await refreshManagedJobs();
      setTimeout(() => loadGroups(jobId, true), 600);
    } catch (error) {
      notify(error.message, true);
      await loadGroups(jobId, true);
      await refreshManagedJobs();
    }
  }
  ...
  list.addEventListener("click", (event) => {
    ...
    if (retryButton) {
      event.preventDefault();
      event.stopPropagation();
      retryGroup(retryButton.dataset.jobId, retryButton.dataset.groupId);
    }
  });

  list.addEventListener("input", (event) => {
    if (!event.target.matches("[data-group-q]")) return;
    const jobId = event.target.closest(".job-groups").id.replace("job-groups-", "");
    const ui = readGroupUi(jobId);
  ...
  list.addEventListener("change", (event) => {
    if (!event.target.matches("[data-group-status]")) return;
    const jobId = event.target.closest(".job-groups").id.replace("job-groups-", "");
    const ui = readGroupUi(jobId);
  ...
  list.addEventListener("scroll", (event) => {
    if (!event.target.matches(".subtask-table-wrap")) return;
    const jobId = event.target.closest(".job-groups").id.replace("job-groups-", "");
    const ui = readGroupUi(jobId);
    ui.scrollTop = event.target.scrollTop;
    center.groupUi.set(jobId, ui);
  }, true);
  ...
  window.addEventListener("f10-job-created", () => setTimeout(refreshManagedJobs, 100));
  window.addEventListener("f10-backend-ready", refreshManagedJobs);

  window.f10JobCenterRefresh = refreshManagedJobs;
