...
function renderPresets() {
  ...
  byId("rawOptions").style.display = preset.include_raw_pack ? "grid" : "none";
  byId("officialOptions").style.display = preset.run_official_validation ? "grid" : "none";
}

function readPreferences() {
  try {
    return JSON.parse(localStorage.getItem("ashare-f10-visual-execution") || "{}");
  } catch (_) {
    return {};
  }
}
...
function applyPreferences() {
  ...
  for (const [id, key] of [
    ...
    ["annualYear", "official_annual_year"], ["quarterYear", "official_quarter_year"],
  ]) {
...
function currentConfiguration() {
  ...
  return {
    ...
    official_annual_year: Number(byId("annualYear").value),
    official_quarter_year: Number(byId("quarterYear").value),
  };
}

function savePreferences() {
  const config = currentConfiguration();
  localStorage.setItem("ashare-f10-visual-execution", JSON.stringify({
    ...config,
    preset: state.selectedPreset,
    poll_seconds: Number(byId("pollSeconds").value),
  }));
  toast("已保存本机可视化配置");
}
...
function validateConfiguration(config) {
  ...
  if (!Number.isInteger(config.raw_pack_max_docs) || config.raw_pack_max_docs < 1 || config.raw_pack_max_docs > 5000) throw new Error("Raw Pack文档上限必须是1到5000的整数");
}
...
function renderMetrics() {
  ...
  byId("metrics").innerHTML = [
    ["任务总数", state.jobs.length],
    ["运行中", state.jobs.filter((job) => activeStatuses.has(job.status)).length],
    ["完整完成", counts.COMPLETED || 0],
    ["部分/失败", (counts.PARTIAL || 0) + (counts.FAILED || 0)],
    ["可选阶段警告", warningCount],
  ].map(([label, value]) => `<div class="metric"><span>${escapeHtml(label)}</span><b>${escapeHtml(value)}</b></div>`).join("");
}

function artifactLinks(job) {
  const entries = Object.entries(job.artifacts || {}).filter(([, value]) => value);
  if (!entries.length) return "";
  return entries.map(([kind]) => `<a target="_blank" href="/api/visual-execution/jobs/${encodeURIComponent(job.job_id)}/download/${encodeURIComponent(kind)}">${escapeHtml(artifactLabels[kind] || kind)}</a>`).join("");
}

function stageCard(job, stage) {
  const visual = job.visual || {};
  const status = visual.stage_status?.[stage] || "UNKNOWN";
  const message = visual.stage_messages?.[stage] || "";
  return `<div class="stage"><strong>${escapeHtml(stageLabels[stage])}</strong><span class="tag stage-${escapeHtml(status)}">${escapeHtml(status)}</span><div class="muted">${escapeHtml(message)}</div></div>`;
}
...
function renderJob(job) {
  ...
  return `<article class="job">
    <div class="job-head"><div><strong>${escapeHtml(job.stock_name || job.stock_code)}（${escapeHtml(job.stock_code)}）</strong><div class="job-id">${escapeHtml(job.job_id)}</div></div><span class="tag ${escapeHtml(job.status)}">${escapeHtml(job.status)}</span></div>
    ...
    <div class="muted">配置：并发${escapeHtml(options.workers || "—")}；Raw Pack=${escapeHtml(Boolean(options.include_raw_pack))}；官方验证=${escapeHtml(Boolean(options.run_official_validation))}</div>
...
byId("savePreset").addEventListener("click", savePreferences);
