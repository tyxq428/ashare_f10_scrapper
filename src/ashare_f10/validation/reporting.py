...
class ValidationReportWriter:
    ...
    def write(
        ...
        official_facts: list[OfficialFact],
        ...
        summary: dict[str, Any] = {
            ...
            "logic_check_counts": dict(logic_counts),
            "ttm_check_counts": dict(ttm_counts),
            "facts_by_report_date": dict(extracted_by_report),
            "high_severity_mismatch_count": len(high_severity),
            "manual_review_required": bool(high_severity),
            "acceptance_status": self._acceptance_status(
                documents, official_facts, reconciliation, logic_checks, ttm_checks
            ),
            "artifacts": {
                "validation_detail_parquet": str(detail_path),
                "official_facts_parquet": str(official_path),
                "validation_evidence_json": str(evidence_path),
                "validation_mismatches_excel": str(excel_path),
                "source_hashes_json": str(hashes_path),
            },
        }
        ...
        hashes = {
            item.local_path: {
                ...
                "url": item.url,
                "sha256": item.sha256,
                "size_bytes": Path(item.local_path).stat().st_size if item.local_path else 0,
            }
        ...
        return ValidationArtifacts(
            self.output_dir, summary_path, detail_path, official_path, evidence_path, excel_path, hashes_path
        )
    ...
    def _format_excel(path: Path) -> None:
        ...
        header_fill, pass_fill, fail_fill = (
            ...
            if sheet.title in {"Reconciliation", "LogicChecks", "TTMChecks", "ParseSuspects"}:
                ...
                if status_column:
                    for row in range(2, sheet.max_row + 1):
                        ...
                        cell.fill = (
                            pass_fill
                            if cell.value in {"PASS", "EXACT_MATCH", "WITHIN_ROUNDING"}
                            else fail_fill
                        )
        workbook.save(path)
