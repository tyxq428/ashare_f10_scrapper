...
6. 写 `W00_plan.md`，再执行环境、权限、Secret 名称、网络和输出路径预检。
7. 建立 Draft PR；PR 描述引用 canonical task directory，不复制另一份状态。
8. 将任务拆成可独立验证的工作包；每包明确允许路径、禁止路径、Gate、失败分类和人工门槛。
9. 正常成功路径自动继续。只有根 `AGENTS.md` 定义的真实门槛才能暂停。
