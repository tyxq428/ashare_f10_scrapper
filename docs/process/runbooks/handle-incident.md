...
```text
Devflow State Consistency (main push, success)
...
如果task已DONE但Issue没有出现：

1. 确认exact-main的 `Devflow State Consistency` 是否成功；
2. 查找同一source Run触发的独立 `Devflow Terminal State Notification`；
3. 核对producer Job Summary：
   - `SOURCE_STATE_CONSISTENCY=VALIDATED`；
   - `SOURCE_EVENT_REQUIRED=PUSH`；
   - `SOURCE_BRANCH_REQUIRED=MAIN`；
   - `TERMINAL_NOTIFICATION_DISPATCHES=<n>`；
4. producer没有Bark Secret，也不会执行HTTP；
5. producer失败为fail-open，不能据此撤销DONE；
6. 不要重跑旧State Consistency、producer或Incident；
7. 如需恢复，创建新的受审任务和canonical generation，并依赖稳定 `devflow-task-completed:<task-id>` marker防止重复。

Incident对COMPLETED同时检查generation marker和稳定task marker。任一已经存在时，后续generation、迟到event或Workflow重跑都不得再次发送Bark。
...
- 通知配置、producer、Artifact或索引失败不应触发Auto Recovery，也不应修改任务终态。
...
1. `task_state.yaml`；
2. 最新State Consistency conclusion；
3. 产品Merge SHA；
4. `HANDOFF.md.next_action`；
5. `FINAL_REPORT.md`；
6. 独立terminal producer Run；
7. canonical Issue中的唯一marker；
8. 可用时核验安全Bark回执Artifact，但它不决定任务终态。
