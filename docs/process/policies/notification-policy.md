...
`COMPLETED` 不从普通Workflow success直接产生，也不由State Consistency本体直接dispatch。

独立 `Devflow Terminal State Notification` 只监听：

```yaml
workflow_run:
  workflow: Devflow State Consistency
  conclusion: success
  source_event: push
...
1. checkout原State Consistency Run的精确 `head_sha`；
2. 验证该SHA属于main历史；
3. 使用该SHA第一父提交作为before；
4. 扫描before→head中的canonical task state变化；
5. 只为严格DONE generation发送既有 `devflow_notify` 事件；
6. 不读取Bark Secret、不执行HTTP请求、不写Issue。
...
producer的source验证、scanner和dispatch均为辅助通知阶段并采用fail-open：不能把已经通过的State Consistency或任务终态重新标记失败，也不能进入Auto Recovery。
...
第一类保留精确generation审计；第二类在整个task生命周期内稳定。Incident在发送Bark前检查两者任一是否已存在，因此：

- producer Workflow重跑不会重复通知；
-同一generation重复dispatch不会重复通知；
-恢复generation不会发送第二条完成Bark；
-迟到的旧generation不会覆盖或重复新完成通知。
...
- completion producer不读取Bark Secret，也不直接执行HTTP请求。
...
自动恢复在通知发出前已经完成全部安全尝试。需要用户处理的通知必须明确写出最小人工动作；用户完成该动作后使用 `/resume` 或回到ChatGPT Web提交新的事实，而不是只回复 `/ack`。
...
-独立completion producer已验证source Run并扫描该精确SHA。
