...
`COMPLETED` 不从普通 Workflow success直接产生。`Devflow Terminal State Notification` 只在 `main` 上扫描canonical task state变化，并要求：
...
Workflow重跑会产生相同fingerprint；Incident marker去重后不得重复写Issue或发送Bark。
