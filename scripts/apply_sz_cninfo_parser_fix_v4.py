...
old = '''    text = _NOTE_REFERENCE_PATTERN.sub(" ", text)
...
'''
new = '''    text = _NOTE_REFERENCE_PATTERN.sub(" ", text)
    text = _NUMBER_PATTERN.sub(" ", text)
...
if old not in text and 're.sub(r"[（(][A-Za-z][）)]"' not in text:
    raise SystemExit("label cleanup marker not found")
...
if old not in text and 'if document.source != "CNINFO":' not in text:
    raise SystemExit("CNINFO table preference marker not found")
text = text.replace(old, new, 1)
...
if old not in text and 'base_score = 140 if document.source == "CNINFO" else 60' not in text:
    raise SystemExit("CNINFO text preference marker not found")
text = text.replace(old, new, 1)
...
def _document() -> OfficialDocument:
    return OfficialDocument(
        "CNINFO", "002352", "2025年年度报告", "2026-03-31", "2025-12-31",
        "annual", "original", "https://example.invalid/report.pdf",
    )


def _fact(field_key: str, label: str, statement_type: str, text: str):
    target = TargetField(
        field_key, label, statement_type, (label,), (field_key,),
        ("RPT_F10_FINANCE_GCASHFLOW",),
    )
    return PdfStatementParser((target,))._extract_from_text(
        text, target, _document(), 1, ("千元", 1000.0), "consolidated",
    )


def test_note_column_after_label_does_not_block_preceding_value() -> None:
    fact = _fact(
        "INVEST_PAY_CASH", "投资支付的现金", "cash_flow",
...
def test_nonbusiness_rows_use_preceding_values() -> None:
    income = _fact(
        "NONBUSINESS_INCOME", "营业外收入", "income_statement",
        "(52)(a) 386,539 311,972 – –\\n加：营业外收入 四\\n"
        "(52)(b) (232,129) (373,060) – –\\n减：营业外支出 四",
    )
    expense = _fact(
        "NONBUSINESS_EXPENSE", "营业外支出", "income_statement",
        "(52)(a) 386,539 311,972 – –\\n加：营业外收入 四\\n"
        "(52)(b) (232,129) (373,060) – –\\n减：营业外支出 四\\n"
