...
def validate_file(path: Path) -> list[str]:
    ...
    if path.as_posix().endswith(
        ...
        if "output-file:" in text:
            errors.append(
                f"{path}: official action output must be handed off "
                "through final-message, not an absolute output-file"
            )
        ...
        if "effort: low" in text:
            errors.append(
                f"{path}: production Codex sessions must use "
                "xhigh reasoning"
            )
    ...
    if path.name == "devflow-branch-gc.yml":
        ...
        if "environment: agent-runtime" in text or "secrets.AGENT_" in text:
            errors.append(
                f"{path}: branch GC must be secret-free"
            )
    ...
    if path.name == "devflow-product-gate.yml":
        ...
        if "environment: agent-runtime" in text:
            errors.append(
                f"{path}: product gate must not access relay "
                "Environment Secrets"
            )
        ...
        if "--base origin/main" in prefix:
            errors.append(
                ...
                "not a moving main tip"
            )
        if "Notify only when automatic merge is genuinely blocked" in text:
            errors.append(
                f"{path}: merge failures must flow through Auto "
                "Recovery, not notify directly"
            )
    ...
    if path.name == "devflow-incident.yml":
        ...
        if "workflow_run:" in text:
            errors.append(
                f"{path}: Incident must not notify directly from "
                "raw workflow failures"
            )
    ...
    if path.name == "devflow-post-merge.yml":
        ...
        if "environment: agent-runtime" in text:
            errors.append(
                f"{path}: post-merge must not access relay "
                "Environment Secrets"
            )
