...
def preview_state_v2(value: dict[str, Any]) -> dict[str, Any]:
    ...
    legacy_status = migrated.pop(
        "research_acceptance_status",
        None,
    )
    if not isinstance(legacy_status, str):
        raise StateError(
            "schema v1 migration requires research_acceptance_status"
        )
...
def run_matrix(fixtures_root: Path) -> dict[str, Any]:
    ...
    state_v1 = load_object(
        fixtures_root / FIXTURE_FILES["state_v1_done"]
    )
    state_v2 = load_object(
        fixtures_root / FIXTURE_FILES["state_v2_running"]
    )
    descriptor_v1 = load_object(
        fixtures_root / FIXTURE_FILES["descriptor_v1_low"]
    )
    descriptor_v2 = load_object(
        fixtures_root / FIXTURE_FILES["descriptor_v2_xhigh"]
    )
    descriptor_v2_low = load_object(
        fixtures_root
        / FIXTURE_FILES["descriptor_v2_low_invalid"]
    )
    ...
    record(
        ...
        check_unknown_descriptor_rejected,
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--fixtures-root",
        type=Path,
        default=Path("tests/fixtures/devflow"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("devflow-upgrade-compatibility.json"),
    )
